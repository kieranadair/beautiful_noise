...
- `core/ai.py` — builds the extraction prompt (including the current venue list, so the model can match against known venues) and calls `ai_complete()` server-side in Snowflake. Uses a write-then-read pattern: the AI result is written to `EXTRACTIONS_RAW` first (since `ai_complete` executes server-side inside a `select()`), then read back by the poster's UUID filename.
...
Data flow for an upload: image/PDF → `pdf_to_image_bytes` (PDF only) → `preprocess_image` (rejects malformed/oversized/unexpected-format files via `ImageRejected`) → MD5 dedup check against cached posters → `upload_to_stage` (Snowflake stage) → `run_extraction` (AI_COMPLETE, logged to `EXTRACTIONS_RAW`) → `prepare_review_defaults` (normalise + fuzzy-match against existing bands/venues + infer full date) → logged to `EXTRACTIONS_PROCESSED` for audit → user reviews/edits in a form (multiselects: Headliners, Support Acts, Credits) → semantic duplicate check (same bands+venue+date) → `save_poster` (upserts VENUES/BANDS/CREDITS/EVENTS, MERGEs POSTERS and BAND_EVENTS with `IS_HEADLINER` per band, re-fetches poster_id and MERGEs POSTER_CREDITS) → `get_all_posters` cache cleared.
...
Dimension tables `VENUES`, `BANDS`, `CREDITS` (upserted via MERGE, all-caps text) → `EVENTS` (dedups on name+date+venue via `equal_null`-safe MERGE) → `POSTERS` (one row per upload, FK to event, `md5_hash`, `upload_type` ∈ {RIGHTS_HOLDER, COMMUNITY}) → `BAND_EVENTS` junction (`is_headliner` flag) and `POSTER_CREDITS` junction (many-to-many, poster↔credit, no role). `POSTER_GALLERY_V` pre-joins everything with `ARRAY_AGG` into `BANDS`/`HEADLINERS`/`SUPPORTS`/`CREDITS` arrays — this is what `get_all_posters()` queries. `EXTRACTIONS_RAW`/`EXTRACTIONS_PROCESSED` are audit trails (raw LLM output vs. post-fuzzy-match), joinable on `file_name` — neither has a `poster_id`, which is why they are named for extractions rather than posters. In `EXTRACTIONS_PROCESSED` the unprefixed columns (`BANDS`, `VENUE`, `DATE`, `EVENT_NAME`) mirror `ai.py`'s `RESPONSE_FORMAT` field names verbatim and hold the model's literal output; the `MATCHED_`/`INFERRED_`/`NORMED_` columns hold what the pipeline made of them. `DATE` is VARCHAR because the model returns `MM-DD` with no year. `REQUESTS` holds contact-page submissions for manual admin review. Full column list and grants: `.context/schema.md`.
...
**Credits vs. legacy designer:** as of 2026-07-12 posters carry a multi-valued `CREDITS` list (designers, photographers, illustrators — flat, optional, like `BANDS`). The old single-designer model (`DESIGNERS` table, `POSTERS.DESIGNER_ID`) is **still present but frozen** — new uploads no longer write `DESIGNER_ID` (stays DEFAULT 1/UNKNOWN). As of 2026-08-10 `POSTER_GALLERY_V` no longer joins `DESIGNERS` at all: the join produced a `DESIGNER_NAME` column nothing read, and being an INNER JOIN on a nullable FK it would have silently dropped any poster with a NULL or orphaned `designer_id` out of the gallery.

**Before `DESIGNERS` is dropped, migrate the one poster that has a real designer.** 46 of 47 posters sit on the UNKNOWN default (`designer_id = 1`), but one carries `designer_id = 401` — a genuine designer never surfaced in the UI. Move it into `CREDITS`/`POSTER_CREDITS` first or the attribution is lost silently.

**`POSTER_GALLERY_V` uses a CTE** to pre-aggregate `CREDITS` per poster before the join — required because BANDS/HEADLINERS/SUPPORTS use non-`DISTINCT` `ARRAY_AGG`, so a naive credits join would fan out and duplicate those arrays. This is not theoretical: at least one poster already carries 2 credits, and posters reach 12 bands / 4 headliners / 10 supports. Preserve the CTE structure if rebuilding.

**Rebuilding `POSTER_GALLERY_V` drops its grants** (`CREATE OR REPLACE VIEW`) — always re-`GRANT SELECT` on the view to both the app role and the admin role afterwards, or the deployed app breaks. Role names are in `.context/` (gitignored). The committed DDL for the view is `sql/views/poster_gallery_v.sql`; see `sql/README.md` before touching it.
...
**This repository is public.** Nothing about credentials, roles, privileges or account
identifiers belongs in this file or anywhere else tracked by git. That detail lives in
`.context/` (gitignored) — ask the user rather than inferring or recording it here.

There is no Snowflake CLI or MCP wired up. Any local SQL work goes through a throwaway script
using `.streamlit/secrets.toml` with `snowflake-connector-python` (or `snowflake-snowpark-python`)
plus `cryptography`, following the same PEM→DER pattern as `get_session()` in `core/db.py`. Do not
assume any saved SnowSQL config works — verify before relying on it.

Two habits worth keeping when doing this:
- **`SHOW` commands are free.** They are served by the cloud services layer and need no warehouse,
  so audits and inspection cost nothing. `SELECT` and `GET_DDL` do resume compute.
- **Committed schema lives in `sql/`**, regenerated by `sql/dump_ddl.py`. Read `sql/README.md`
  first — those files are a record, not a migration, and `GET_DDL` emits `CREATE OR REPLACE`.

Privilege scoping is under review and is not settled; treat the current grants as provisional and
confirm intent with the user before changing or relying on them.
